...
#define MAX_FRAME_COUNT 2048	// maximum # of left/mono channel data values to process at a time
...
public:
	SIM();
	~SIM();
	void Process( PROCESS_BUFFER_DATA_TYPE *samples, int BUF_SIZE);
	void measure_rms( PROCESS_BUFFER_DATA_TYPE *samples, int BUF_SIZE);
	void reset(void);

	void SetsnrValue(double snrdb) {
		snr = pow(10.0, snrdb/20.0);
	}
	void ReadGains( double& noise_gain, double& signalgain) {
...
private:
	...
	double samplerate;
	double ssum;
	double sim_buffer[MAX_FRAME_COUNT];
	cmplx delay0_buffer[MAX_FRAME_COUNT];
	cmplx delay1_buffer[MAX_FRAME_COUNT];
	cmplx delay2_buffer[MAX_FRAME_COUNT];
	CDelay* delay;
	CPath* path0;
	CPath* path1;
	CPath* path2;
