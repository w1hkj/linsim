...
// Copyright(C) 1996-1998 Takuya OOURA <ooura@mmm.t.u-tokyo.ac.jp>
//----------------------------------------------------------------------
// Copyright 2006-2012 Dave Freese W1HKJ <w1hkj@w1hkj.com>
//----------------------------------------------------------------------
...
void Cfft::SetFFTParams(int ave, double gain, int type )
{
	...
	if(ave>0)
		...
		AveSize = 1;
	if( (LastAve != ave) || (LastLogMode != LogMode) )
		ResetFFT();
	LastLogMode = LogMode;
	LastAve = AveSize;
	Gain = 0.1*(gain*10.0/(10.0-Clip));
...
bool Cfft::GetFFTData(int start, int stop, long* OutBuf )
{
	for( int i=start; i<=stop; i++ )		//copy and scale into OutBuf[]
		OutBuf[i] = (long)(Gain*pFFTAveBuf[i<<1]);
	return Overload;
}
...
double tmp;
	...
	if(LogMode)
	{
		for (k = 2; k < m; k += 2 ) 
		{
			...
			if( (tmp -= Clip)<0.0 )
				pFFTAveBuf[k] = (1.0-1.0/AveSize)*pFFTAveBuf[k];
			else
 				pFFTAveBuf[k] = (1.0-1.0/AveSize)*pFFTAveBuf[k] +
									(1.0/AveSize)*tmp;
			tmp = log10(xr+K_C) + K_B;
			if( (tmp -= Clip)<0.0 )
				pFFTAveBuf[j] = (1.0-1.0/AveSize)*pFFTAveBuf[j];
			else
 				pFFTAveBuf[j] = (1.0-1.0/AveSize)*pFFTAveBuf[j] +
									(1.0/AveSize)*tmp;
		}
	...
	else
	{
		for (k = 2; k < m; k += 2 ) 
		{
			...
			pFFTAveBuf[k] = (1.0-1.0/AveSize)*pFFTAveBuf[k] + 
					(1.0/AveSize)*pow(xi , K_ROOT)/K_ROOTGN;
 			pFFTAveBuf[j] = (1.0-1.0/AveSize)*pFFTAveBuf[j] +
					(1.0/AveSize)*pow(xr , K_ROOT)/K_ROOTGN;
		}
	}
